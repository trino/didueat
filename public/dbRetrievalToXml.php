...
$query = sprintf("SELECT id,name,slug,email,website,phone,formatted_address,postal_code, lat, lng, ( 3959 * acos( cos( radians('%s') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('%s') ) + sin( radians('%s') ) * sin( radians( lat ) ) ) ) AS distance, description,logo,delivery_fee,minimum,rating from restaurants WHERE open=1 AND status=1".$genreSQL." HAVING distance < '%s' ORDER BY distance LIMIT 0 , 20",
