...
function additemtoreceipt(menu_id, ids, quantity, price, csr_action, app_title, extratitle, dbtitle) {
    log("menu_id " + menu_id + " ids: " + ids + " quantity: " + quantity + " price: " + price + " csr_action: "
        + csr_action + " apptitle: " + app_title + " extratitle: " + extratitle + " dbtitle: " + dbtitle);
    $.post(baseurl + "/ajax", {
        type: "handlemenu",
...
function updatequantity(menuitem_id) {
    ...
    $.post(baseurl + "/ajax", {
        ...
        if (quantity == 0) {
            $("#menuitem_" + menuitem_id).fadeOut(500, function () {
                $(this).remove();
            });
...
function checkout() {
    ...
    if (Number($('#subtotal1').val()) == 0) {
        ...
        if (allowbypassminumum) {
            ...
            if (!confirm("Minimum delivery fee not met! Bypass anyway? (DEBUG MODE)")) {
                return false;
            }
        } else {
