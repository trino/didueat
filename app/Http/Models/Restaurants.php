    ...
    public static function searchRestaurants($data = '', $per_page = 10, $start = 0, $ReturnSQL = false, &$count = 0) {
        ...
        foreach(array("tags", "cuisine") as $field) {//LIKE
            if (isset($data[$field]) && $data[$field]) {
                //DISABLE CUSINE SEARCH (Disable in the form, not the code)
        ...
        if(isset($data["delivery-time"]) && $data["delivery-time"]){
            $data["delivery-datetime"] = $data["delivery-time"];
            $data["date"] = strtotime($data["delivery-datetime"]) ;
            $data["delivery-time"] = date('H:i:s', $data["date"]);
            $date = now(true, $data["date"]);
        }
