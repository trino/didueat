<?php
    ...
    if(isset($query) && $count > 0 && is_iterable($query)){
        foreach($query as $value){
            ...
            ?> @include("dashboard.restaurant.restaurantpanel", array("Restaurant" => $value, "order" => true, "is_menu" => isset($is_menu))) <?php
            if($is_open){
...
<div id="loadMoreBtnContainer">
    @if($hasMorePage > 0)
        <div class="row">
            <div class="col-md-12 ">
                <button id="loadingbutton" data-id="{{ $start }}" align="center" class="loadMoreRestaurants btn btn-secondary-outline btn-lg btn-block" title="{{ $alts["loadmore"] }}">Load More ...</button>
