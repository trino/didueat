...
<?php echo newrow($new, "Search Address", "", false); ?>
    ...
    @else
        <DIV CLASS="nowrap">
            ...
            &nbsp;
            <a class="btn btn-primary headerbutton" id="header-search-button" onclick="geolocate(formatted_address);" style="position:absolute;top:0px;">
...
<?= newrow($new, "City", "", $required, 4); ?>
    <input type="text" id="city" name="city" class="form-control"
           {{ $is_disabled }} {{$required}} value="{{ (isset($addresse_detail->city))?$addresse_detail->city:old('city') }}" {{$required}}>
</div></div>

<?= newrow($new, "Province", "", true, 4); ?>
...
<?= newrow($new, "Postal Code", "", true, 4); ?>
    <input type="text" name="postal_code" id="postal_code" {{ $is_disabled }} class="form-control"
           placeholder="Postal Code"
           value="{{ (isset($addresse_detail->postal_code))?$addresse_detail->postal_code: old('postal_code') }}">
</div></div>

<?= newrow($new, "Country", "", true, 4); ?>
    <select name="country" id="country" class="form-control" {{ $is_disabled }} id="country2"
            {{$required}} onOLDchange="provinces('{{ addslashes(url("ajax")) }}', '{{ (isset($addresse_detail->province))?$addresse_detail->province: old('province') }}');">
...
<?php
    if(!$restSignUp){
            ...
            echo '<hr width="100%" align="center" /><button type="submit" class="btn btn-primary" style="margin-left:auto;margin-right:auto;">Save</button></div></div>';
    }
    
    if($isUser){
        echo newrow($new, "Apartment/Unit", "", false, 4); ?>
        ...
        <?= newrow($new, "Buzz Code", "", false, 4); ?>
        <input type="text" name="buzz" class="form-control" {{ $is_disabled }} placeholder="Buzz/ringer/doorbell Code"
               value="{{ (isset($addresse_detail->buzz))?$addresse_detail->buzz:old('buzz') }}">
        </div></div>

        <?= newrow($new, "Notes", "", false, 9); ?>
        <input type="text" name="notes" class="form-control" {{ $is_disabled }} placeholder="ie: Side door"
