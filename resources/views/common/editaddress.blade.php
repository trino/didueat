...
enter restaurnat address
<input style="" type="text" name="formatted_address" id="formatted_address" class="form-control formatted_address" placeholder="Address, City or Postal Code" value="
...
<?= newrow($new, "Find My Location"); ?>
        ...
        @else
                ...
                <a class="btn btn-primary" oldstyle="display: none;" id="header-search-button" onclick="geolocate(formatted_address);" style="">
                    <h4 class="m-a-0 p-a-0"><i class="fa fa-map-marker"></i></h4>
                </a>
...
<HR>
        @endif
<?= newrow($new, "Address"); ?>
        <input type="text" id="rout_street_number" {{ $is_disabled }} name="address" class="form-control" placeholder="" value="{{ (isset($addresse_detail->address))?$addresse_detail->address: old('address') }}" {{$required}}>
    </div>
</div>
...
<?= newrow($new, "Province"); ?>
        ...
        <!--input type="text" id="province" name="province" class="form-control" {{$required}} value="{{ (isset($addresse_detail->province))?$addresse_detail->province:old('province') }}"-->
            ...
            <input type="text" name="postal_code" id="postal_code" {{ $is_disabled }} class="form-control" placeholder="M5V2B2" value="{{ (isset($addresse_detail->postal_code))?$addresse_detail->postal_code: old('postal_code') }}">
            </div>
            </div>

            <?= newrow($new, "Phone"); ?>
            <input type="text" name="phone" class="form-control" {{ $is_disabled }} placeholder="416555555" value="{{ (isset($addresse_detail->phone))?$addresse_detail->phone: old('phone') }}" {{$required}}>
            </div>
            </div>
            <?= newrow($new, "Notes"); ?>
            <input type="text" name="notes" class="form-control" {{ $is_disabled }} placeholder="ie: Apartment/Unit/Buzz Code/Side door" value="{{ (isset($addresse_detail->notes))?$addresse_detail->notes:old('notes') }}">
            </div>
            </div>
            <?
    if(isset($disclaimer)){
