<?php
printfile("views/common/signupform.blade.php");
if (!isset($new)) {
    $new = false;
}
?>
...
@if(!read("id") || isset($user_detail))
    ...
    <input type="text" name="name" class="form-control" id="name" placeholder="Full Name"
           value="{{ (isset($user_detail->name))?$user_detail->name: priority(old('name'), read('name')) }}" required>
    </div></div>

    <?= newrow($new, "Email"); ?>
    <input type="email" name="email" class="form-control" id="email" placeholder="Email Address"
           value="{{ (isset($user_detail->email))?$user_detail->email: priority(old('email'), read('email')) }}"
           required onkeyup="validationOnkeyup()">
    </div></div>
    ...
    <input type="password" name="confirm_password0" class="form-control" id="confirm_password0"
           placeholder="Re-type Password" required onkeyup="validationOnkeyup()">
    </div></div>
    ...
    <div class="col-sm-12">
        <LABEL>
            <input type="checkbox" name="subscribed" id="subscribed" value="1"
                   @if(isset($user_detail->subscribed) && $user_detail->subscribed == "1") checked @endif />
