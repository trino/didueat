...
@if(\Session::has('session_profiletype') && \Session::get('session_profiletype') == 1)
    <div class="form-group row">
        ...
        <div class="col-sm-9">
            <LABEL>
                <input type="radio" name="user_type" value="restaurant" onclick="switchdivs(this.value);"
                   @if(isset($credit_cards_list->user_type) && $credit_cards_list->user_type == 'restaurant') checked @endif >
                Restaurant
            </LABEL>
            <LABEL>
                <input type="radio" name="user_type" value="user" onclick="switchdivs(this.value);"
    ...
    <div class="form-group row restaurant_id" style="display: {{ (isset($credit_cards_list->user_id) && $credit_cards_list->user_type == 'restaurant')? 'block' : 'none' }}">
        <label for="restaurant_id" class="col-sm-3">Assign Restaurant </label>
        <div class="col-sm-9">
            <div class="input-icon">
                <select name="select_restaurant_id" id="restaurant_id" class="form-control">
                    @foreach($restaurants_list as $restaurant)
                        <option value="{{$restaurant->id}}" @if(isset($credit_cards_list->user_id) && $credit_cards_list->user_id == $restaurant->id) 'selected' @else '' @endif>{{$restaurant->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
    </div>
    <div class="form-group row user_id" style="display: {{ (isset($credit_cards_list->user_id) && $credit_cards_list->user_type == 'user')? 'block' : 'none' }}">
        <label for="user_id" class="col-sm-3">Assign User </label>
        <div class="col-sm-9">
            <div class="input-icon">
                <select name="select_user_id" id="user_id" class="form-control">
                    @foreach($users_list as $user)
                        <option value="{{$user->id}}" @if(isset($credit_cards_list->user_id) && $credit_cards_list->user_id == $user->id) 'selected' @else '' @endif>{{$user->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
