...
<div id="is_delivery_options"
     style="display: {{ (isset($restaurant->is_delivery) && $restaurant->is_delivery > 0)?'block':'none' }};">
...
<label><SPAN class="is_delivery_2b">Hours Open:</SPAN></label>
...
echo '<label class="col-sm-12"></label>';

echo '<label class="col-sm-3 row">Delivery Times</u>:</label>
<div class="col-sm-9"><LABEL id="is_delivery_optionsCB"><input type="CHECKBOX" ' . $is_disabled . ' onclick="same(event);" ID="samehours" name="samehours"' . iif($isthesame, " checked") . '>&nbsp; Same as Regular Hours</LABEL></div>';
...
<div class="row {{ $class }}">
    ...
    <div class="col-sm-{{ $width }}">
        <input type="text" name="{{$value}}_open{{$suffix}}" id="open{{$suffix}}[{{ $key }}]" value="{{ $opentime }}"
               title="Open" class="form-control time" onfocus="this.blur();"/>
    </div>
    <div class="col-sm-1">to</div>
    <div class="col-sm-{{ $width }}">
        <input type="text" name="{{$value}}_close{{$suffix}}" id="close{{$suffix}}[{{ $key }}]" value="{{ $closetime }}"
               title="Close" class="form-control time" onfocus="this.blur();"/>
    </div>
</div>
<?php
}
